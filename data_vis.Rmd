...
transparent_color1 <- ("#440154FF", alpha = 0.5) 

  
raster_colors <- c("#FFFFCC", "#A1DAB4", "#41B6C4", "#225EA8")

raster_alpha <- c(0.5, 0.5, 1, 1)
...
predicted_conflict_raster <- 
  tm_shape(model_conflict_raster) +
  tm_raster(breaks= seq(0, 1, by= 0.1),
            palette = "viridis",
            title = "Probability (%)") + # order =
  tmap_mode(mode = "view") +
...
model_conflict_CRS <- st_crs(model_2030_conflict_raster)
...
```{r}
conflict_2030_raster <- 
  tm_shape(model_2030_conflict_raster) +
